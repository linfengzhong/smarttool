...
function install_prerequisite () {
	...
	if [[ -f "/etc/fuckGFW/prerequisite/check" ]]; then
		...
		$installType wget lsof tar unzip curl socat bind9-utils jq >/dev/null 2>&1
...
#   https://raw.githubusercontent.com/linfengzhong/smarttool/main/Website/html1.zip
	...
	if [[ -d "/etc/fuckGFW/website/html" && -f "/etc/fuckGFW/website/html/check" ]]; then
		...
		if [[ "${nginxBlogInstallStatus}" == "y" ]]; then
			...
			wget -P /etc/fuckGFW/website https://raw.githubusercontent.com/linfengzhong/smarttool/main/Website/html${randomNum}.zip
...
function set_current_uuid {
	...
	if [[ -f "$HOME/.myHostUUID" ]]; then
		print_error "已经设置服务器UUID，无需重复设置！"
...
define command {
    command_name    check_port_9443
    command_line    \$USER1\$/check_tcp -H \$HOSTADDRESS$ -p 9443 -w 0.2 -c 0.5 -t 5
}
...
function menu() {
	...
	echoContent yellow "11.安装 prerequisite | 12.安装 acme.sh | 13.安装 bpytop"
	...
	echoContent yellow "35.show configs [Sub Menu] | 36.show logs [Sub Menu]"
